...
// --- Versión secuencial ---
long double ejercicio1(long double x) {
...
// --- Versión multihilo ---
long double ejercicio1hilos(long double x, int P) {
    long double y = (x - 1) / (x + 1);
    long long chunk = M / P;
...
int main() {
    ...
    // --- Secuencial ---
    double t1 = seconds();
    long double res1 = ejercicio1(x);
    double t2 = seconds();

    std::cout << std::setprecision(15)
              << "\n[Secuencial] Resultado: " << res1
              << "\nTiempo: " << (t2 - t1) << " s\n";

    // --- Multihilo ---
    double t3 = seconds();
    long double res2 = ejercicio1hilos(x, P);
    double t4 = seconds();

    std::cout << std::setprecision(15)
              << "\n[Multihilo] Resultado: " << res2
              << "\nTiempo: " << (t4 - t3) << " s\n";
